...
- **（新規）リポジトリ直下の`launch.json`・`settings.json`削除**：用途不明の古い設定ファイル（`.vscode/`配下ではない、VSCode自体は読み込まない死んだ設定）と判明し`git rm`でステージング済みだが、**2026-09-15時点でまだコミットされていない**（ユーザーが意図的に保留、準備ができたタイミングでコミット判断）。`.vscode/launch.json`・`.vscode/settings.json`（実際にVSCodeが使う設定）は無事残存・内容変更なしを確認済み。
